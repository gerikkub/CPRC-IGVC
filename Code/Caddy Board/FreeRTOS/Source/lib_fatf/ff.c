...
uint32_t get_fattime (void)
{
	...
	xRTCArray CurrTimeDate;

	if( getDateTimeDS1307( &CurrTimeDate ) == pdTRUE ) // set to zero if date can't be read.
	{
    ...
    CurrTimeDate.Year   = (uint8_t)(2013 - 1980);			// just using the current date today...
    CurrTimeDate.Month  = 3;
    CurrTimeDate.Date   = 10;
    CurrTimeDate.Hour   = 12;
    CurrTimeDate.Minute = 34;
    CurrTimeDate.Second = 56;

#endif

    return ((uint32_t)CurrTimeDate.Year << 25) |			// Microsoft: Y0 = 1980.
           ((uint32_t)CurrTimeDate.Month << 21) |
           ((uint32_t)CurrTimeDate.Date << 16) |
           ((uint32_t)CurrTimeDate.Hour << 11) |
           ((uint32_t)CurrTimeDate.Minute << 5) |
           (((uint32_t)CurrTimeDate.Second >> 1) << 0);
...
#if _WORD_ACCESS == 1
	while (cnt >= sizeof(int16_t)) {
		*(int16_t*)d = *(int16_t*)s;
		d += sizeof(int16_t); s += sizeof(int16_t);
		cnt -= sizeof(int16_t);
	}
#endif
	while (cnt--)
...
/* Fill memory */
static
void mem_set (void* dst, int16_t val, uint16_t cnt) {
	uint8_t *d = (uint8_t*)dst;
...
static
int16_t mem_cmp (const void* dst, const void* src, uint16_t cnt) {
...
static
int16_t chk_chr (const char* str, int16_t chr) {
...
int16_t ff_cre_syncobj (	/* TRUE:Function succeeded, FALSE:Could not create due to any error */
	uint8_t vol,			/* Corresponding logical drive being processed */
	xSemaphoreHandle *sobj		/* Pointer to return the created sync object */
)
{
	int16_t ret;

	*sobj = xSemaphoreCreateMutex();		/* FreeRTOS */
	ret = (int16_t)(*sobj != NULL);
...
int16_t ff_del_syncobj (	/* TRUE:Function succeeded, FALSE:Could not delete due to any error */
	xSemaphoreHandle sobj		/* Sync object tied to the logical drive to be deleted */
)
{
	int16_t ret;
...
int16_t ff_req_grant (	/* TRUE:Got a grant to access the volume, FALSE:Could not get a grant */
	xSemaphoreHandle sobj	/* Sync object to wait */
)
{
	int16_t ret;

	ret = (xSemaphoreTake(sobj, (_FS_TIMEOUT  / portTICK_RATE_MS )) == pdTRUE);	/* FreeRTOS */
...
void ff_rel_grant (
	xSemaphoreHandle sobj	/* Sync object to be signalled */
)
{
...
FRESULT chk_lock (	/* Check if the file can be accessed */
	...
	int16_t acc			/* Desired access (0:Read, 1:Write, 2:Delete/Rename) */
)
{
	uint16_t i, be;
...
static
int16_t enq_lock (void)	/* Check if an entry is available for a new file */
{
	uint16_t i;
...
static
uint16_t inc_lock (	/* Increment file open counter and returns its index (0:int16_t error) */
	DIR* dj,	/* Directory object pointing the file to register or increment */
	int16_t acc		/* Desired access mode (0:Read, !0:Write) */
)
{
	uint16_t i;
...
FRESULT dir_next (	/* FR_OK:Succeeded, FR_NO_FILE:End of table, FR_DENIED:Could not stretch */
	...
	int16_t stretch		/* 0: Do not stretch table, 1: Stretch table if needed */
)
{
	uint32_t clst;
	uint16_t i;
...
FRESULT dir_alloc (
	DIR* dj,	/* Pointer to the directory object */
	uint16_t nent	/* Number of contiguous entries to allocate (1-21) */
)
{
	FRESULT res;
	uint16_t n;
...
static
int16_t cmp_lfn (			/* 1:Matched, 0:Not matched */
...
static
int16_t pick_lfn (			/* 1:Succeeded, 0:Buffer overflow */
...
FRESULT dir_read (
	DIR *dj,		/* Pointer to the directory object */
	int16_t vol			/* Filtered by 0:file/dir or 1:volume label */
)
{
	FRESULT res;
	uint8_t a, c, *dir;
...
)
{
	...
	if ((uint16_t)*path < ' ') {			/* Null path means the start directory itself */
		res = dir_sdi(dj, 0);
		dj->dir = 0;
...
)
{
	...
	*rfs = fs;							/* Return pointer to the corresponding file system object */
	...
	/* Following code attempts to mount the volume. (analyse BPB and initialise the fs object) */
	...
	/* An FAT volume is found. Following code initialises the file system object */
...
)
{
	...
	*br = 0;	/* Clear read byte counter */
	...
	for ( ;  btr;								/* Repeat until all data read */
		...
		rcnt = SS(fp->fs) - ((uint16_t)fp->fptr % SS(fp->fs));	/* Get partial sector data from sector buffer */
...
)
{
	...
	*bw = 0;	/* Clear write byte counter */
	...
	for ( ;  btw;							/* Repeat until all data written */
		...
		wcnt = SS(fp->fs) - ((uint16_t)fp->fptr % SS(fp->fs));/* Put partial sector into file I/O buffer */
...
#endif
#endif
			if (!w || chk_chr("\"*+,.:;<=>\?[]|\x7F", w) || j >= (uint16_t)((w >= 0x100) ? 10 : 11)) /* Reject invalid chars for volume label */
...
FRESULT f_forward (
	FIL *fp, 							/* Pointer to the file object */
	uint16_t (*func)(const uint8_t*,uint16_t),	/* Pointer to the streaming function */
	uint16_t btf,						/* Number of bytes to forward */
	uint16_t *bf						/* Pointer to number of bytes forwarded */
)
{
	FRESULT res;
	...
	*bf = 0;	/* Clear transfer byte counter */
	...
	remain = fp->fsize - fp->fptr;
	if (btf > remain) btf = (uint16_t)remain;			/* Truncate btf by remaining bytes */
...
FRESULT f_mkfs (
	...
	uint16_t au			/* Allocation unit size [bytes] */
)
{
...
#if _USE_ERASE	/* Erase data area if needed */
	{
		...
		if (disk_ioctl(pdrv, CTRL_ERASE_SECTOR, eb) != RES_OK)
			return FR_DISK_ERR;
	}
#endif
...
{
	uint16_t i, n, sz_cyl, tot_cyl, b_cyl, e_cyl, p_cyl;
...
TCHAR* f_gets (
	...
	int16_t len,	/* Size of string buffer (characters) */
	FIL* fp			/* Pointer to the file object */
)
{
	int16_t n = 0;
...
)
{
	...
	for (cc = res = 0; cc != EOF; res += cc) {
		...
		if (c == 'l' || c == 'L') {	/* Prefix: Size is long int16_t */
			f |= 4; c = *str++;
		}
		if (!c) break;
		d = c;
		...
		case 'S' :					/* String */
			...
			continue;
		case 'C' :					/* Character */
			cc = f_putc((TCHAR)va_arg(arp, int16_t), fp); continue;
		case 'B' :					/* Binary */
			r = 2; break;
		case 'O' :					/* Octal */
			r = 8; break;
		...
		v = (f & 4) ? (uint32_t)va_arg(arp, long) : ((d == 'D') ? (uint32_t)(long)va_arg(arp, int16_t) : (uint32_t)va_arg(arp, uint16_t));
		if (d == 'D' && (v & 0x80000000)) {
			v = 0 - v;
			f |= 8;
		}
...
#endif // define only if portSD_CARD is enabled.
